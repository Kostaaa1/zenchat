      ...
      {is_image ? (
        ...
        <div className="flex max-w-[70%] items-center justify-center break-all leading-tight">
          {rounded2 && !isLoggedUserASender && (
            <Avatar size="sm" image_url={activeChatroom?.users.find((x) => x.user_id === sender_id)?.image_url} />
          )}
          <p
            className={cn(
              "rounded-[3px] px-3 py-2",
              isLoggedUserASender ? "rounded-l-3xl bg-lightBlue" : "ml-9 rounded-r-3xl bg-neutral-700",
      ...
      {activeMessage?.id === id || isHovered ? (
        ...
        >
          ...
          {activeMessage?.id === id ? (
            ...
            >
              <p className="text-sm text-neutral-200">{convertAndFormatDate(created_at)}</p>
