...
const NavHamburger: FC<NavHamburgerProps> = ({ list }) => {
  ...
  useOutsideClick([iconRef, dropdownRef], "click", () => setShowDropdown(false))

  return (
    <div className="relative" onClick={() => setShowDropdown(!showDropdown)} ref={iconRef}>
      <NavListItem variant={list} title={isResponsive ? "" : "Menu"}>
        <Icon name="Menu" color="white" size="28px" />
      </NavListItem>
      <AnimatePresence>{showDropdown && <UserDropdown dropdownRef={dropdownRef} />}</AnimatePresence>
    </div>
  )
}
