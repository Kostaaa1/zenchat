...
const Dropzone: FC<Props> = ({ onDrop }) => {
  ...
  return (
    <div ref={drop} className="flex h-full w-full flex-col items-center justify-center space-y-2">
      ...
      <Button className="relative cursor-pointer" buttonColor="blue">
        <label className="absolute w-full cursor-pointer opacity-0" htmlFor="fileInput">
          <input id="fileInput" type="file" onChange={(e) => onDrop(e.target.files![0])} />
        </label>
...
const DndUploadModal = forwardRef<HTMLDivElement>((_, ref) => {
  ...
  const [modalTitle, setModalTitle] = useState<"Create new post" | "Processing" | "Post uploaded">("Create new post")
  ...
  const triggerFileError = () => {
    toast.error("Error selecting the video. The format may not be supported or file may be corrupted.")
    setFile(null)
    setMediaSrc(null)
  }
      ...
      <Modal>
        ...
        >
          <div className="relative flex h-12 w-full items-center justify-between border-[1px] border-x-0 border-t-0 border-b-neutral-600 p-2">
            <Icon className={cn(!mediaSrc ? "hidden" : "")} name="ArrowLeft" onClick={clear} />
            <p className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 font-semibold">{modalTitle}</p>
            {file && modalTitle === "Create new post" ? (
              ...
              <Icon className="absolute right-0 top-0 -translate-y-1/2" name="X" onClick={clearAndClose} />
            )}
          </div>
          {!isUploading ? (
            <>
              {mediaSrc && file ? (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  transition={{ ease: "easeInOut", duration: 0.3 }}
                  className={cn("relative flex h-full w-full justify-between", isMobile && "w-full flex-col")}
                >
                  <div className="w-full">
                    {file.type.startsWith("image/") && <img src={mediaSrc} alt={mediaSrc} />}
                  ...
                  <div
                    className={cn(
                      "h-full",
                      isMobile ? "absolute bottom-0 h-[36vw] w-full bg-[#282828]" : "min-w-[280px]"
                    )}
                  >
