...
uploadRouter.post("/post", multerUploadPost.array("post"), async (req, res) => {
  console.log("uploading post called");
  if (!req.files || !req.body) return;
  const files = req.files as Express.Multer.File[];
  const { serialized } = req.body as {
    serialized: string;
  };
  const mediaFile = files[0] as Express.Multer.File;
  const deserialized = JSON.parse(serialized);

  const fileName = AWS_BUCKET_URL + "posts/" + mediaFile.originalname;
  ...
  console.log("Deserieazl", deserialized);
